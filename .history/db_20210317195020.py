...
async def get_winner(db, auction_id):
    ret = []
    async with db.execute("SELECT Player_id FROM bids WHERE Auction_id = ? AND bid_value = (SELECT MAX(bid_value) FROM bids WHERE Auction_id = ?)", [auction_id, auction_id]) as cursor:
        async for row in cursor:
            ret.append(row[0])
