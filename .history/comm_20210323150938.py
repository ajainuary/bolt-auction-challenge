...
async def complete_auction(db, auction):
    result = await finish_auction(db, auction.id)
    await broadcast(db, request.config_dict["client"], {"type": "END", "Auction_id": auction_id, "Winner": result[0], "Winning_Value": result[1]}, auction.id)
