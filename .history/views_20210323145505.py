...
from db import get_leaderboard, get_logs, get_player_ids, get_auctions, insert_player, insert_auction, insert_bid, finish_auction
...
async def end_auction(request):
    ...
    asyncio.gather(*list(map(async lambda x: await complete_auction(request.config_dict["DB"], x), filter(lambda x: x['open'], auctions))))
    return web.json_response(
        {
            "status": "OK"
        }
