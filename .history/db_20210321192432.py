...
async def get_leaderboard(db):
    ret = []
    async with db.execute("SELECT Name, SUM(value) FROM transactions, players WHERE players.Id = transactions.Player_id GROUP BY transactions.Player_id ORDER BY SUM(value) ASC") as cursor:
        async for row in cursor:
            ret.append(
                {
                    "name": row[0],
...
async def get_winner(db, auction_id):
    ...
    await db.commit()
    await db.execute("INSERT INTO transactions (Player_id, value)", [result[0], 1])
    await db.commit()
    await log(db, {
        "player_id": result[0],
        "auction_id": auction_id,
