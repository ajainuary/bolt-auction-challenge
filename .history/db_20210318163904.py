...
async def get_winner(db, auction_id):
    ret = []
    async with db.execute("SELECT Player_id, bid_value FROM bids WHERE Auction_id = ? AND bid_value = (SELECT MAX(bid_value) FROM bids WHERE Auction_id = ?)", [auction_id, auction_id]) as cursor:
        async for row in cursor:
    ...
    await db.execute(
        "INSERT INTO  logs (Player_id, Auction_id, Desc, Severity) SELECT bids.Player_id, bids.Auction_id, printf(\"Paid %f\",MAX(bids.bid_value)), \"Notify\" FROM bids WHERE bids.Auction_id = ? AND bids.Timestamp < (SELECT bids2.Timestamp FROM bids AS bids2 WHERE bids2.Auction_id = ? AND bids2.Player_id = ? AND bids2.bid_value = (SELECT MAX(bids3.bid_value) FROM bids AS bids3 WHERE bids3.Auction_id = ? AND bids3.Player_id = ?) LIMIT 1) GROUP BY bids.Player_id",
        [auction_id, auction_id, result[0], auction_id, result[0]],
    )
    await db.commit()
    return result
