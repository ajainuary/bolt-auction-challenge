...
from db import get_leaderboard, get_logs, get_player_ids, insert_player, insert_auction, insert_bid, get_winner
...
async def new_auction(request):
    ...
    await get_player_ids(request.config_dict["DB"], players)
    return web.json_response({
        "id": 1
    })
