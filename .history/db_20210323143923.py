...
async def get_auction(db, key):
    async with db.execute('SELECT Id, Status FROM auctions WHERE Key = ?', key) as cursor:
        return list(map(lambda x: {"id": x[0], "status": x[1]}, await cursor.fetchall()))
